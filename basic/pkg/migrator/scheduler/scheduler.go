...
import (
	...
	"compus_blog/basic/pkg/gormx/connpool"
	logger2 "compus_blog/basic/pkg/logger"
	"compus_blog/basic/pkg/migrator"
	"compus_blog/basic/pkg/migrator/events"
	"compus_blog/basic/pkg/migrator/validator"
	"context"
	"fmt"
...
func (s *Scheduler[T]) RegisterRoutes(server *gin.RouterGroup) {
	// 将这个暴露为 HTTP 接口
	// 你可以配上对应的 UI
	server.POST("/src_only", ginx2.Wrap(s.SrcOnly))
	server.POST("/src_first", ginx2.Wrap(s.SrcFirst))
	server.POST("/dst_first", ginx2.Wrap(s.DstFirst))
	server.POST("/dst_only", ginx2.Wrap(s.DstOnly))
	server.POST("/full/start", ginx2.Wrap(s.StartFullValidation))
	server.POST("/full/stop", ginx2.Wrap(s.StopFullValidation))
	server.POST("/incr/stop", ginx2.Wrap(s.StopIncrementValidation))
...
// SrcOnly 只读写源表
func (s *Scheduler[T]) SrcOnly(c *gin.Context) (ginx2.Result, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) SrcFirst(c *gin.Context) (ginx2.Result, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) DstFirst(c *gin.Context) (ginx2.Result, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) DstOnly(c *gin.Context) (ginx2.Result, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Scheduler[T]) StopIncrementValidation(c *gin.Context) (ginx2.Result, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	s.cancelIncr()
	return ginx2.Result{
...
func (s *Scheduler[T]) StopFullValidation(c *gin.Context) (ginx2.Result, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	s.cancelFull()
	return ginx2.Result{
...
func (s *Scheduler[T]) StartFullValidation(c *gin.Context) (ginx2.Result, error) {
	// 可以考虑去重的问题
	s.lock.Lock()
	defer s.lock.Unlock()
	// 取消上一次的
	...
	go func() {
		...
		err := v.Validate(ctx)
		if err != nil {
			s.l.Warn("退出全量校验", logger2.Error(err))
		}
	}()
	return ginx2.Result{
		Msg: "OK",
