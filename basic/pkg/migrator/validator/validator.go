...
import (
	...
	events2 "compus_blog/basic/pkg/migrator/events"
	"context"
...
func (v *Validator[T]) baseToTarget(ctx context.Context) error {
	...
	for {
		...
		case nil:
			v.dstDiff(ctx, src)
		default:
			v.l.Error("src => dst 查询源表失败", logger2.Error(err))
		}
		offset++
	}
}
...
func (v *Validator[T]) dstDiff(ctx context.Context, src T) {
	...
	default:
		v.l.Error("src => dst 查询目标表失败", logger2.Error(err))
	}
}
...
func (v *Validator[T]) targetToBase(ctx context.Context) error {
	...
	for {
		...
		default:
			v.l.Error("dst => src 查询目标表失败", logger2.Error(err))
		}
...
func (v *Validator[T]) srcMissingRecords(ctx context.Context, ts []T) {
	...
	default:
		v.l.Error("dst => src 查询源表失败", logger2.Error(err))
	}
}
