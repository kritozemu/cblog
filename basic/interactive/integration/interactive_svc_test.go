...
import (
	...
	"compus_blog/basic/interactive/integration/startup"
...
func (s *InteractiveTestSuite) TestIncrReadCnt() {
	...
	intr := grpc.NewInteractiveServiceServer(svc)
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp, err := intr.IncrReadCnt(context.Background(), &intrv1.IncrReadCntRequest{
...
func (s *InteractiveTestSuite) TestLike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp, err := intr.Like(context.Background(), &intrv1.LikeRequest{
...
func (s *InteractiveTestSuite) TestDislike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp, err := intr.CancelLike(context.Background(), &intrv1.CancelLikeRequest{
				Biz: tc.biz, BizId: tc.bizId, Uid: tc.uid,
			})
			assert.Equal(t, tc.wantErr, err)
			assert.Equal(t, tc.wantResp, resp)
...
func (s *InteractiveTestSuite) TestCollect() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp, err := intr.Collect(context.Background(), &intrv1.CollectRequest{
...
func (s *InteractiveTestSuite) TestGet() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			res, err := intr.Get(context.Background(), &intrv1.GetRequest{
				Biz: tc.biz, BizId: tc.bizId, Uid: tc.uid,
			})
			assert.NoError(t, err)
			assert.True(t, proto.Equal(tc.wantRes, res))
		})
	}
}
...
func (s *InteractiveTestSuite) TestGetByIds() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			res, err := intr.GetByIds(context.Background(), &intrv1.GetByIdsRequest{
