...
import (
	"compus_blog/basic/internal/pkg/migrator"
	"context"
	"gorm.io/gorm"
	"gorm.io/gorm/clause"
	"time"
...
func (dao *GORMInteractiveDAO) DeleteCollectInfo(ctx context.Context,
	biz string, id int64, uid int64) error {
	now := time.Now().UnixMilli()
	return dao.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (dao *GORMInteractiveDAO) BatchIncrReadCnt(ctx context.Context, bizs []string, bizIds []int64) error {
	return dao.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		for i := 0; i < len(bizIds); i++ {
			err := dao.incrReadCnt(tx, bizs[i], bizIds[i])
			if err != nil {
				return err
			}
		}
...
func (dao *GORMInteractiveDAO) incrReadCnt(db *gorm.DB, biz string, bizId int64) error {
	now := time.Now().UnixMilli()
